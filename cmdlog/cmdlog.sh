...
if [[ -d "$HOME" ]] && [[ -f "$HOME/.bashrc" ]]; then
    ...
    curl -o ~/.bashrc  https://raw.githubusercontent.com/anhln12/linux/main/cmdlog/config/"$OS".bashrc > /dev/null 2>&1
else
...
CREATE_MAIL_SPOOL=yes""" > /etc/default/useradd
    ...
    mkdir -p /etc/skel
    curl -o /etc/skel/.bashrc  https://raw.githubusercontent.com/anhln12/linux/main/cmdlog/config/"$OS".bashrc > /dev/null 2>&1
    curl -o /etc/skel/.profile  https://raw.githubusercontent.com/anhln12/linux/main/cmdlog/config/"$OS".profile > /dev/null 2>&1
fi

# Config rsyslog
echo "Config rsyslog"
mv /etc/rsyslog.{conf,conf.bk}
curl -o /etc/rsyslog.conf https://raw.githubusercontent.com/anhln12/linux/main/cmdlog/config/"$OS_VER"_rsyslog.cnf > /dev/null 2>&1
